...
msg_t read_acceleration(int16_t *values) {
	...
	if ( status == RDY_OK && (buffer_rx[0] & 0x8) ){
		values[0] = ((int16_t)buffer_rx[1]<<8)|buffer_rx[2];
		values[1] = ((int16_t)buffer_rx[3]<<8)|buffer_rx[4];
		values[2] = ((int16_t)buffer_rx[5]<<8)|buffer_rx[6];
	}else{
...
msg_t read_magetometer(int16_t *values) {
	uint8_t buffer_rx[7];
	...
	if ( status == RDY_OK /*&& (buffer_rx[6] & 0x1) */){//does not seems to work
		values[0] = ((int16_t)buffer_rx[0] << 8) | buffer_rx[1];
		values[2] = ((int16_t)buffer_rx[2] << 8) | buffer_rx[3];
		values[1] = ((int16_t)buffer_rx[4] << 8) | buffer_rx[5];
	}else{
		values[0] = values[1] = values[2] = 0;
	}
